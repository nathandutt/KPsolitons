...
# Spatial grid
x_range = y_range
x_s = np.linspace(-x_range, x_range, n_steps)
...
y_s = np.linspace(-y_range, y_range, n_steps)
...
norm = colors.SymLogNorm(
    linthresh=1e-8,   # size of linear region around 0
    linscale=1,
    vmin=-Zmax,
    vmax=Zmax
)
surf= ax.contour(X, Y, Z, levels=100, norm=norm, cmap="bone")
