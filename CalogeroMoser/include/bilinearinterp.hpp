...
inline int roots(const double c0, const double c1, const double c2, sol_array& y_sols){
     ...
     if(disc < 0.) return 0;
     double sol_1 = 1./(2.*c0) * (-c1 + std::sqrt(disc));
     double sol_2 = 1./(2.*c0) * (-c1 - std::sqrt(disc));
     int returned = 0;
     if((0.<=sol_1) && (sol_1 <= 1.)){
...
inline addif(int idx, const double y_root, const array& ga, sol_array& x_sols){
    //Adds x_root corresponding to y root,only if within correct range
    //returns 0 if none added, 1 if added
    double x_candidate = (ga[0]+y_sol*ga[2])/(ga[1] + y_sol*ga[2]);
...
inline std::vector<Point> getZeros(const array& xc, const array& yc, const array& phi_x, const array& phi_y){
   array gx; array gy;
